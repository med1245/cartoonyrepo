...
class FaselHDX : MainAPI() {
    ...
    override suspend fun loadLinks(
        ...
        // ── Stage 1: Download link (fastest, no WebView) ────────────────────
        ...
        // ── Stage 2: loadExtractor (handles known video hosts) ───────────────
        try { loadExtractor(iframeSrc, data, subtitleCallback, callback) } catch (_: Exception) {}

        // ── Stage 3: HTTP + cfKiller → scan JS for m3u8/mp4 ─────────────────
        try {
            ...
            if (!m3u8.isNullOrBlank()) {
                M3u8Helper.generateM3u8(name, m3u8, referer = mainUrl).forEach(callback)
                return true
            }
        ...
        // ── Stage 4: WebView with CF cookies pre-injected into CookieManager ─
        // Pre-warming cfKiller on the iframe URL stores the CF clearance cookie.
        // We then inject it into Android's shared WebView CookieManager so that
        // the WebViewResolver's internal WebView is already authenticated and
        // won't hit a Cloudflare challenge page.
        return try {
            // 1. Let cfKiller solve Cloudflare for the player URL via HTTP
            try {
                app.get(iframeSrc, referer = data, interceptor = cfKiller,
                    headers = mapOf("User-Agent" to USER_AGENT, "Referer" to data))
            } catch (_: Exception) {}

            // 2. Inject the solved CF cookies into the WebView's cookie store
            val cookieHeader = cfKiller.getCookieHeaders(iframeSrc)["Cookie"] ?: ""
            if (cookieHeader.isNotBlank()) {
                val cm = CookieManager.getInstance()
                cm.setAcceptCookie(true)
                cookieHeader.split(";").forEach { kv ->
                    val part = kv.trim()
                    if (part.isNotEmpty()) cm.setCookie(iframeSrc, part)
                }
                cm.flush()
            }

            // 3. Run WebViewResolver – the WebView now has CF clearance cookies
